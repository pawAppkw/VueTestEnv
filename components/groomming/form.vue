  ...
  props: ['itemData', 'categories'],
    ...
    methods: {
      ...
      async addData(formItems) {
        const data = await this.$axios.$post('vendor/groomers_services', formItems)
        try {
      ...
      async updateData(formItems) {
        const data = await this.$axios.$post(`vendor/groomers_services/${this.itemData.id}`, formItems)
        try {
          if(data.success === true){
