...
export default {
  props: ['itemData'],
    ...
    methods: {
      ...
      async addData() {
        const data = await this.$axios.$post('vendor/pharmacy_drugs_categories', this.itemData)
        try {
      ...
      async updateData() {
        ...
        const data = await this.$axios.$post(`vendor/pharmacy_drugs_categories/${this.itemData.id}`, form)
        try {
          if(data.success === true){
