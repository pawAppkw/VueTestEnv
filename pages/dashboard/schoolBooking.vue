    ...
    <v-col cols="12">
      <v-card elevation="0">
        <v-toolbar elevation="0">
          <v-toolbar-title class="mr-3">School Booking</v-toolbar-title>
...
export default {
  ...
  methods: {
    ...
    async changeStatus() {
      await this.$axios.$post(`vendor/change-schools-booking-status/${this.itemData.id}`, {
    ...
    async changeStatus(item, status) {
      await this.$axios.$post(`vendor/change-schools-booking-status/${item.id}`, {
        status: status
      }).then(res => {
        if (res.success === true) {
    ...
    async getAllData() {
      const data = await this.$axios.$get('vendor/schools-booking')
