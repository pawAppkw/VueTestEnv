<template>
  <v-row>
    <v-col cols="12">
      <v-card elevation="0">
        <v-toolbar elevation="0">
          <v-toolbar-title>Pharmacies Categories</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-text-field
            v-model="search"
          ...
          <v-btn class="ml-3" color="primary" outlined @click="addItem()"><v-icon>mdi-plus</v-icon>New Pharmacies Category</v-btn>
        </v-toolbar>
        <v-data-table
          :headers="headers"
          :items="items"
    ...
    <v-dialog width="600" v-model="dialog">
      <v-card>
        <v-card-title>
          {{ addDialog == 1 ? 'Add Pharmacies Category' : 'Edit Pharmacies Category'}}
        </v-card-title>
        <v-card-text>
...
<script>
import categoryForm from '../../components/pharmacies-category/form.vue'
export default {
...
methods: {
  ...
  async deleteItem() {
    const data = await this.$axios.$delete(`vendor/pharmacy_drugs_categories/${this.itemData.id}`)
