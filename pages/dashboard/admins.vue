        ...
        <v-col cols="12">
          <v-card elevation="0">
            <v-toolbar elevation="0">
              ...
              <v-btn class="ml-3" color="primary" outlined @click="addItem()"><v-icon>mdi-plus</v-icon> New Admin</v-btn>
            </v-toolbar>
            <v-data-table
              :headers="headers"
              :items="admins"
...
export default {
  ...
  async deleteItem() {
    const data = await this.$axios.$delete(`vendor/delete-admin/${this.itemData.id}`)
  ...
  errorRequest (event) {
    ...
    async getAllData() {
    const data = await this.$axios.$get('vendor/admins')
